...
import { SYMBOL, TOKENS } from '../../config/config';
  ...
  const makeBooking = async (values: BookingFormValues) => {
    try {
      ...
      if(token && token == 'MATIC' ){
        ...
        transaction = await exchangeContract.newRequest(creatorId, JSON.stringify(requestData),ERC20_CONTRACTS[token],parseUnits(values.amount) );
        }
       toast.dismiss();
                    ...
                    <>
                      ...
                      <div style={{ marginBottom: 40 }}>
                        <TextField
                          ...
                          endText={token}
                          type="number"
                          placeholder={price && price != 0 ? price + '+' : formatETH(convertToFloat(creator.price)) + '+' }
